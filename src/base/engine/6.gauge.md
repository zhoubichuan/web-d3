...
export default {
  ...
  methods: {
    init() {
      ...
      const radius =
        d3.min([chart.getBodyWidth() * 0.95, chart.getBodyHeight() * 0.95]) / 2;
        ...
        const drawAngles = config.domain.map((value, i, domain) => {
          ...
          if (i !== domain.length - 1) {
            return {
              ...
              endAngle:
                (((domain[i + 1] / config.totalValue) * config.totalAngle -
                  config.totalAngle / 2) *
                  Math.PI) /
                180,
            };
      ...
      chart.renderTicks = function () {
        const drawAngles = [];
        for (
          let i = -config.totalAngle / 2;
          i <= config.totalAngle / 2 + 0.01;
          i += config.totalAngle / 50
        ) {
        ...
        function drawTicks(d, i) {
          if (i === 0 || i === 50) return;
          const innerRadius =
            i % 5 === 0 ? radius - config.width : radius - config.width / 3;
          d3.select(this)
            .append("line")
        ...
        function rotateTween(d) {
          ...
          return function (t) {
            return (
              ...
              ") " +
              "rotate(" +
              ((lastAngle + angleDiff * t) / config.totalValue - 0.5) *
                config.totalAngle +
              ")"
            );
          };
