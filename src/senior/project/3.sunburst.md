...
export default {
  ...
  methods: {
    init() {
      ...
      d3.json("/web-d3/basicSunburst.json").then(function (data) {
        /* ----------------------------配置参数------------------------  */
        ...
        chart.scaleXToAngle = d3
          .scaleLinear()
          .domain([0, chart.getBodyWidth()])
          .range([0, Math.PI * 2]);

        chart.scaleYToRadius = d3
          .scaleLinear()
          .domain([0, chart.getBodyHeight()])
          .range([
            0,
            d3.min([chart.getBodyWidth(), chart.getBodyHeight()]) / 2,
          ]);
        ...
        chart.renderText = function () {
          ...
          chart.slicesEnter
            ...
            .attr("transform", (d) => {
              let rotateAngle =
                (((chart.scaleXToAngle(d.x0) + chart.scaleXToAngle(d.x1)) / 2) *
                  180) /
                Math.PI;
          ...
          function getTextLocation(d, type) {
            //获取文本的x和y坐标
            let middleRadius = 0;
            let middleAngle = 0;
            ...
            if (chart.scaleYToRadius(d.y0) > 0) {
              middleAngle =
                (chart.scaleXToAngle(d.x0) + chart.scaleXToAngle(d.x1)) / 2;
              middleRadius =
                (chart.scaleYToRadius(d.y1) + chart.scaleYToRadius(d.y0)) / 2;
            }
        ...
        chart.addMouseOn = function () {
          d3.selectAll(".slice").on("click", function (d) {
            if (d.data.id === chart.currentRoot) {
              //点击中心节点回退
              if (d.parent) {
