...
export default {
  ...
  methods: {
    init() {
      ...
      d3.json("/web-d3/data.json").then(function (data) {
        /* ----------------------------配置参数------------------------  */
        ...
        chart.scaleY = d3
          .scaleLinear()
          .domain([
            (Math.floor(d3.min(data.map((d) => d.min)) / 100) - 1) * 100,
            (Math.floor(d3.max(data.map((d) => d.max)) / 100) + 1) * 100,
          ])
          .range([chart.getBodyHeight(), 0]);

        /* ----------------------------渲染盒子和须----------------------  */
        chart.renderBoxs = function () {
          let groups = chart.body().selectAll(".g").data(data);

          let groupsEnter = groups.enter().append("g").attr("class", "g");
          ...
          groupsEnter.each(function () {
            for (let i = 0; i < 5; i++) {
              d3.select(this).append("line").attr("stroke", config.barStroke);
            }
          });
