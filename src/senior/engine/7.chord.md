...
export default {
  ...
  methods: {
    init() {
      ...
      d3.json("/web-d3/chord.json").then(function (data) {
        /* ----------------------------配置参数------------------------  */
        ...
        chart.renderText = function () {
          ...
          d3.selectAll(".g")
            ...
            .attr(
              "x",
              (d) => radius * Math.sin((d.endAngle + d.startAngle) / 2)
            )
            .attr(
              "y",
              (d) => -radius * Math.cos((d.endAngle + d.startAngle) / 2)
            )
            .attr(
              "transform",
              (d) =>
                "rotate(" +
                (180 * (d.endAngle + d.startAngle)) / 2 / Math.PI +
                "," +
                radius * Math.sin((d.endAngle + d.startAngle) / 2) +
                "," +
                -radius * Math.cos((d.endAngle + d.startAngle) / 2) +
                ")"
            )
        ...
        chart.renderRibbon = function () {
          const ribbon = d3.ribbon().radius(config.innerRadius);
          ...
          ribbons
            ...
            .attr("fill", (d) =>
              d3.rgb(chart._colors(d.target.index % 10)).brighter()
            )
            .attr("stroke", (d) =>
              d3.rgb(chart._colors(d.target.index % 10)).darker()
            )
            .attr("d", ribbon);
        };
        ...
        chart.addMouseOn = function () {
              ...
              d3.selectAll(".ribbons path")
                .filter(
                  (link) =>
                    ...
                    link.source.index !== d.index
                )
                .classed("show", true);
            })
            .on("mouseleave", function () {
              d3.selectAll(".ribbons path").classed("show", false);
            });
          ...
          d3.selectAll(".ribbons path")
            ...
            .on("mouseleave", function () {
              ...
              d3.selectAll(".g path").classed("show", false);
            });
        };
